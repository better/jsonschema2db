...
class JSONSchemaToDatabase(ABC):
    """JSONSchemaToDatabase is the mother class for everything

    :param schema: The JSON schema, as a native Python dict
    :param database_flavor: One of ["postgres", "redshift", "snowflake"]
    :param drop_schema: (optional) Whether or not to drop (with CASCADE) the target schema
    :param schema_namespace: (optional) A string denoting a schema (namespace) under which all tables will be created
    ...
    def __init__(
        ...
        prefix_col_name="prefix",
        abbreviations={},
        extra_columns=[],
        root_table="root",
        s3_client=None,
        s3_bucket=None,
        s3_prefix="jsonschema2db",
        s3_iam_arn=None,
    ):
        ...
        self._links = {}
        self._backlinks = {}
        self._schema = schema_namespace
        self._drop_schema = drop_schema
        ...
        self._translation_tree = self._traverse(
            schema, schema, table=self._root_table, comment=schema.get("comment")
        )
        ...
        self._table_columns = {}
        max_column_length = {"postgres": 63, "redshift": 127, "snowflake": 251}[
            self._database_flavor
        ]
        ...
        for table, column_types in self._table_definitions.items():
            for column in column_types.keys():
                if len(column) > max_column_length:
                    warnings.warn(
                        "Ignoring_column because it is too long: %s.%s"
                        % (table, column)
                    )
            columns = sorted(
                col for col in column_types.keys() if 0 < len(col) <= max_column_length
            )
            self._table_columns[table] = columns

    def _table_name(self, path):
        return "__".join(
            change_case.ChangeCase.camel_to_snake(self._abbreviations.get(p, p))
            for p in path if p
        )
    ...
    def _traverse(
        ...
        while "$ref" in tree:
            ...
            for elem in p:
                if elem not in tree:
                    warnings.warn(
                        "%s.%s: Broken definition: %s"
                        % (table, self._column_name(path), ref)
                    )
                    return
                tree = tree[elem]
            ...
            definition = p[
                -1
            ]  # TODO(erikbern): we should just make this a boolean variable
        ...
        if special_keys:
            ...
            for p in special_keys:
                for q in tree[p]:
                    res.update(
                        self._traverse(schema, q, path, table, json_path=new_json_path)
                    )
            ...
            if "comment" in tree:
                self._column_comments.setdefault(table, {})[
                    self._column_name(path)
                ] = tree["comment"]
            ...
            if "patternProperties" in tree:
                ...
                if len(tree["patternProperties"]) > 1:
                    warnings.warn(
                        "%s.%s: Multiple patternProperties, will ignore all except first"
                        % (table, self._column_name(path))
                    )
                for p in tree["patternProperties"]:
                    ref_col_name = table + "_id"
                    res["*"] = self._traverse(
                        schema,
                        tree["patternProperties"][p],
                        tuple(),
                        self._table_name(path),
                        (table, ref_col_name, self._column_name(path)),
                        tree.get("comment"),
                        new_json_path + (p,),
                    )
                    break
                ...
                if definition:
                    ...
                    for p in tree["properties"]:
                        res[p] = self._traverse(
                            schema,
                            tree["properties"][p],
                            (p,),
                            self._table_name([definition]),
                            (table, ref_col_name, self._column_name(path)),
                            tree.get("comment"),
                            new_json_path + (p,),
                        )
                    self._table_definitions[table][ref_col_name] = "link"
                    self._links.setdefault(table, {})[ref_col_name] = (
                        "/".join(path),
                        self._table_name([definition]),
                    )
                else:
                ...
                warnings.warn(
                    "%s.%s: Object with neither properties nor patternProperties"
                    % (table, self._column_name(path))
                )
        
        elif tree["type"] == "array":
            self._table_definitions[table][self._column_name(path)] = "array"
            if "comment" in tree:
                self._column_comments.setdefault(table, {})[
                    self._column_name(path)
                ] = tree["comment"]
            res = {"_column": self._column_name(path), "_type": "array"}
        else:
            if tree["type"] == "null":
                res = {}
                ...
                warnings.warn(
                    "%s.%s: Type error: %s"
                    % (table, self._column_name(path), tree["type"])
                )
                res = {}
            else:
    ...
    def _coerce_type(self, t, value):
        ...
        try:
            if t == "number":
                ...
                if self._database_flavor == 'snowflake':
                    return True, "PARSE_JSON(' %s ')" % json.dumps(value).replace('\'', '\\\'')
                else:
                    warnings.warn(
                        "Ignoring unsupported column with type: %s"
                        % (t)
                    )
                    pass
        except:
    ...
    def _insert_items_generate_rows(self, items, extra_items, count):
        ...
        for item_id, data in items:
            ...
            for path, value in data:
                ...
                # if type(value) == list:
                    # for v in value:
                    #     self._insert_items_generate_rows(v, extra_items, count)
                    ...
                    if "*" in subtree:
                        ...
                        if count:
                            self.failure_count[path] = (
                                self.failure_count.get(path, 0) + 1
                            )
                        break
                    else:
            ...
            for table, table_values in res.items():
                for prefix, row_values in table_values.items():
                    row_array = [item_id, prefix] + [
                        row_values.get(t) for t in self._table_columns[table]
                    ]
    ...
    def create_tables(self, con):
        ...
        with con.cursor() as cursor:
            ...
            for table, columns in self._table_columns.items():
                ...
                create_q = (
                    'create table %s (id %s, "%s" %s not null, "%s" text not null, %s unique ("%s", "%s"), unique (id))'
                    % (
                ...
                if table in self._table_comments:
                    self._execute(
                        cursor,
                        "comment on table %s is %%s" % self._get_table_name(table),
                        (self._table_comments[table],),
                    )
                for c in columns:
                    if c in self._column_comments.get(table, {}):
                        self._execute(
                            ...
                            % (self._get_table_name(table), c),
                            (self._column_comments[table][c],),
                        )
        ...
        with con.cursor() as cursor:
            ...
            for table, data in data_by_table.items():
                ...
                self._execute(
                    ...
                    % (self._get_table_name(table).encode(), cols.encode(), args),
    ...
    def analyze(self, con):
        ...
        with con.cursor() as cursor:
            for table in self._table_columns.keys():
                self._execute(cursor, "analyze %s" % self._get_table_name(table))


class JSONSchemaToRedshift(JSONSchemaToPostgres):
    ...
    def insert_items(self, con, items, extra_items={}, mutate=True, count=False):
        """ Inserts data into database.
        :param con: redshift connection object
        :param items: is an iterable of tuples `(item id, values)` where `values` is either:
        - A nested dict conforming to the JSON spec
        - A list (or iterator) of pairs where the first item in the pair is a tuple specifying the path, and the second value in the pair is the value.
        :param extra_items: A dictionary containing values for extra columns, where key is an extra column name.
        ...
        with tempfile.TemporaryDirectory() as tmpdirname, con.cursor() as cursor:
            ...
            for table, fn in temp_files.items():
                ...
                if self._debug:
                    print(
                        "Uploading data for table %s from %s (%d bytes) to %s"
                        % (table, fn, os.path.getsize(fn), s3_path),
                        file=sys.stderr,
                    )
                self._s3_client.upload_file(
                    Filename=fn, Bucket=self._s3_bucket, Key=s3_path
                )

                query = (
                    "copy %s from 's3://%s/%s' csv %s truncatecolumns compupdate off statupdate off"
                    % (
                        self._get_table_name(table),
                        self._s3_bucket,
                        s3_path,
                        self._s3_iam_arn and "iam_role '%s'" % self._s3_iam_arn or "",
                    )
                )
