...
def test_lff():
    ...
    translator = JSONSchemaToPostgres(
        ...
        abbreviations={"AbbreviateThisReallyLongColumn": "AbbTRLC",},
        debug=True,
    )
        ...
        [
            (
                "loan_file_abc123",
                {
                    ...
                    "RealEstateOwned": {
                        "1": {
                            "Address": {"City": "Brooklyn", "ZipCode": "65432"},
                            "RentalIncome": 1000,
                        },
    ...
    assert list(
        query(
            con,
            "select loan_file_id, prefix, loan__amount, subject_property__acreage, subject_property__address__latitude, loan__abb_trlc from schm.root",
        )
    ) == [("loan_file_abc123", "", 500000, 42.0, 43.0, None)]
    assert set(
        query(
            con, "select loan_file_id, prefix, city, zip_code from schm.basic_address"
        )
    ) == set(
        [
            ("loan_file_abc123", "/SubjectProperty/Address", "New York", "12345"),
    ...
    assert set(
        query(
            con,
            "select loan_file_id, prefix, rental_income from schm.real_estate_owned",
        )
    ) == set(
        [
            ("loan_file_abc123", "/RealEstateOwned/1", 1000),
    ...
    assert set(
        query(
            con,
            "select subject_property__address_id from schm.root union select address_id from schm.real_estate_owned",
        )
    ) == set(query(con, "select id from schm.basic_address"))
    assert set(query(con, "select root_id from schm.real_estate_owned")) == set(
...
def test_pp_to_def():
    ...
    translator.insert_items(
        ...
        [
            (
                33,
                [
                    (("aBunchOfDocuments", "xyz", "url"), "http://baz.bar"),
    ...
    assert translator.failure_count == {
        ("moreDocuments", "abc"): 1,
        ("moreDocuments", "abc", "url"): 1,
    }
    ...
    assert list(
        query(con, "select id, prefix, item_id, root_id from a_bunch_of_documents")
    ) == [(1, "/aBunchOfDocuments/xyz", 33, 1)]
    assert set(query(con, "select prefix, url, item_id from file")) == set(
        [
    ...
    assert set(
        ...
        + list(query(con, "select file_id from more_documents"))
    ) == set([(1,), (2,)])
...
def test_time_types():
    ...
    assert list(query(con, "select id, d from root")) == [
        (1, datetime.date(2018, 7, 8)),
        (2, datetime.date(2013, 3, 2)),
    ]
    assert list(
        (id, ts.isoformat()) for id, ts in query(con, "select id, ts from root")
